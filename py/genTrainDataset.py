...
neg_num = 1
total=0
train_dataset_num = 110000
test_dataset_num = int(train_dataset_num/10)
vaild_dataset_num = int(train_dataset_num/10)

dir_folder = "./features/"
func_list_file = dir_folder + "function_list_1"+PREFIX+".csv"
train_file = dir_folder + "train_1"+PREFIX+".csv"
test_file = dir_folder + "test_1"+PREFIX+".csv"
vaild_file = dir_folder + "vaild_1"+PREFIX+".csv"
...
with open(func_list_file, "r") as fp:
    for line in csv.reader(fp):
        ...
        if line[0]  in func_list_dict:
            ...
            #print line
            value = line[3]+"." + line[2]
            func_list_arr.append(line[0])#函数名
            func_list_dict.setdefault(line[0],value) #函数名：版本+地址
