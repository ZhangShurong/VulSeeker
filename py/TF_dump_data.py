...
import numba
import itertools
from genTrain_dump_data import *
# ===========  global parameters  ===========
...
# TEST_TFRECORD="bug_data/test_dump_data_"+"5765"+PREFIX+".tfrecord"
#
# print (TEST_TFRECORD)
...
def load_all_data():
    ...
    for file in all_result:
        block_path = file + "/block_info" + PREFIX + ".csv"
        adj_info_path=file + "/adj_info" + PREFIX + ".txt"
        basename = os.path.basename(file)
        block_num, cur_func_block_dict = load_block_info(block_path, feature_dict , basename)

        if is_debug:
            print ("load cfg ...")
        # load every function's cfg
        load_graph(adj_info_path, block_num, cur_func_block_dict,basename,
               uid_graph)
...
def generate_adj_matrix_pairs(uid_pair_list):
    ...
    count = 0
    #graph_bug = uid_graph_1[uid_pair_list[0][0]]
    #adj_arr_bug = np.array(nx.convert_matrix.to_numpy_matrix(graph_bug, dtype=float))
    #adj_str_bug = adj_arr_bug.astype(np.string_)
    # 扁平化列表
	...
	graph_bug = uid_graph_1[uid_pair_list[0][0]]
        ...
        adj_str_bug = adj_arr_bug.astype(np.string_)
    # 扁平化列表
        adj_matrix_all_1.append(",".join(list(itertools.chain.from_iterable(adj_str_bug))))
        graph_all = uid_graph[uid_pair[1]]   #解决标签为1的情况
        adj_arr_all = np.array(nx.convert_matrix.to_numpy_matrix(graph_all, dtype=float))
        adj_str_all = adj_arr_all.astype(np.string_)
        adj_matrix_all_2.append(",".join(list(itertools.chain.from_iterable(adj_str_all))))
...
def generate_features_pair(uid_pair_list):
    ...
    uid = uid_pair_list[0][0]
    node_list = uid_graph_1[uid].nodes()
    uid_prefix = uid.split('.')[0]  # 从右边第一个'.'分界，分成两个字符串 即 identifier, function_id
    node_vector_bug = []
    for node in node_list:
        node_vector_bug.append(feature_dict_1[str(uid_prefix)][0][node])
    node_length.append(len(node_vector_bug))
    node_arr_bug = np.array(node_vector_bug)
    node_str_bug = node_arr_bug.astype(np.string_)
    for uid_pair in uid_pair_list:
        if is_debug:
            count += 1
            print ("         %04d feature, [ %s , %s ]"%(count, uid_pair[0], uid_pair[1]))

        num1.append(len(node_vector_bug))
        node_vector_all_1.append(",".join(list(itertools.chain.from_iterable(node_str_bug))))
...
# ======================= construct test data =====================
seek_pair=[]
bug_result=['./QQ_bug/bugs/5A712F2F5A000']
print(bug_result)
for file in bug_result:
    ...
    with open(func_list_file, "r") as fp:
        for line in csv.reader(fp):
            ...
            #test_file = file + '/' + bug_key + ".csv"
            TEST_TFRECORD = "QQ_bug/tf_result_1/" + bug_key+"_264553"  + ".tfrecord"
            print("---------------Dealing:",TEST_TFRECORD,"-----------------")

 	    test_file="./QQ_bug/bugs/5A712F2F5A000/a.csv"
            test_pair, test_label = load_dataset(test_file)
            ...
            if not seek_pair:
                for pair in test_pair:
                    seek_pair.append(pair[1])
                str_pair=np.array(seek_pair).astype(np.string_)
            ...
            for item1,item2,item3,item4,item5,item6, item7, item8, item9 in itertools.izip(
                    ...
                    test_num1, test_num2, node_list ,str_pair):
